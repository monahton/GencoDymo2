...
GenocDymo2 runs in the R statistical computing environment. You will need R version 4.1.0 or higher. The most recent version of GencoDymo2 can be installed from GitHub. We recommend using the pak package for fast and reliable installation.
...
lnc_46_df <- load_file("~/Desktop/gencode.v46.long_noncoding_RNAs.gtf.gz")
head(lnc_46_df)

# Load mouse GFF3
mouse_pri_36 <- load_file("~/Desktop/gencode.vM36.primary_assembly.annotation.gff3.gz")
head(mouse_pri_36)
```
...
To quantify changes between two releases, use <span style="color:coral">`compare_release()`</span>. Specify the two annotation data frames and the type of feature to compare ("gene", "transcript", "exon", etc.). Additional filters like the gene_type and the baseline can allow to focus on subsets (e.g., only TEC or ptotein_coding genes) and different reference counts for the calculation of the difference percentage.
...
motifs_donor <- extract_ss_motif(
  ...
  verbose = T,
  save_fasta = T,
  output_file = "~/Desktop/lnc_47_5ss_motifs.fa"
)
...
motifs_acc <- extract_ss_motif(
  ...
  verbose = T,
  save_fasta = T,
  output_file = "~/Desktop/lnc_47_3ss_motifs.fa"
)
```
...
mouse_cds_seqs <- extract_cds_sequences(
  ...
  save_fasta = TRUE,
  output_file = "~/Desktop/mouse_pri_36_CDS.fa.gz",
  verbose = TRUE
)
```
...
5. Abou Alezz, M., Celli, L., Belotti, G., et al. (2020). _GC-AG Introns Features in Long Non-coding and Protein-Coding Genes Suggest Their Role in Gene Expression Regulation_. Front. Genet. 2020 11:488. https://doi:10.3389/fgene.2020.00488
